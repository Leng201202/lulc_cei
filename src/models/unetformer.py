"""UNetFormer -- a UNet-like transformer for remote sensing segmentation.
...
The architecture is kept as published. The encoder is any timm backbone exposed
through ``features_only``, so ``backbone_name`` selects between the ResNet-18 of
the paper and the ResNet-101 used here.

Note on the return value: in training mode the model returns ``(main, aux)`` --
the segmentation output plus a deeply-supervised auxiliary output. In eval mode
it returns the main output alone. ``src/engine/trainer.py`` handles the tuple;
everything downstream sees a plain tensor.
"""
...
class GlobalLocalAttention(nn.Module):
    """The Global-Local Transformer Block's attention.

    Runs a windowed self-attention branch (global context) alongside a small
    convolution branch (local detail) and sums them, which is the paper's core
    idea: transformer context without losing fine boundaries.
    """
...
class Block(nn.Module):
    ...
    def __init__(self, dim=256, num_heads=16, mlp_ratio=4., qkv_bias=False, drop=0.,
                 attn_drop=0., drop_path=0., act_layer=nn.ReLU6,
...
class WF(nn.Module):
    """Weighted Fusion: learnable-weight combination of skip and upsampled features."""
